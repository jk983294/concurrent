#ifndef _ZERG_TIME_H_
#define _ZERG_TIME_H_
...
inline long GetMicrosecondsSinceEpoch() {
    struct timeval tp;
...
inline struct timeval GetTimevalFromMicrosecondsSinceEpoch(long ms) {
    struct timeval tp;
...
inline struct tm GetTmFromMicrosecondsSinceEpoch(long ms) {
...
const int64_t year = 31536000L * 1000;
const int64_t month = 2592000L * 1000;
const int64_t day = 86400 * 1000;
const int64_t week = 604800 * 1000;
const int64_t hour = 3600 * 1000;
const int64_t minute = 60 * 1000;
const int64_t second = 1 * 1000;
const int64_t millisecond = 1;
const double TradingDay = 1.1;
...
inline uint64_t HumanReadableMicrosecond(const char* interval_string) {
    return HumanReadableMicrosecond(std::string(interval_string));
}

inline uint64_t HumanReadableMillisecond(const std::string& interval_string) {
    return HumanReadableMicrosecond(interval_string) / 1000;
}

inline uint64_t HumanReadableMillisecond(const char* interval_string) {
...
inline uint64_t timespec2nanos(const timespec& ts) {
...
// YYYYMMDD format
inline uint32_t nano2date(uint64_t nano) {
    if (nano == 0) {
...
inline bool string_2_second_intraday(const string& str, const string& format, int& secs) {
...
inline bool string_2_microsecond_intraday(const string& str, const string& format, int& msecs) {
...
inline std::string now_local_string() {
    timeval tv;
    gettimeofday(&tv, nullptr);
...
inline std::string replace_time_placeholder(const std::string& str, int d = 0, int32_t t = -1) {
