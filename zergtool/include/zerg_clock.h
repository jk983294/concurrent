#ifndef ZERG_ZERG_CLOCK_H
#define ZERG_ZERG_CLOCK_H
...
public:
    bool IsTradingDay() {
        if (tradingDays.empty()) THROW_ZERG_EXCEPTION("TradingDayList is empty, cannot decide if trading day.");
        return isTradingDay;
    }
...
public:
    ...
    Clock(const Clock& clock) {
        ...
        if (policy::IsTrading()) {
            if (tradingDays.empty()) THROW_ZERG_EXCEPTION("Tradingday list cannot be empty for trading clock");
        }
    ...
    Clock& operator=(const Clock& clock) {
        ...
        SetTimeInfo();
        if (policy::IsTrading()) {
            if (tradingDays.empty()) THROW_ZERG_EXCEPTION("Trading day list cannot be empty for trading clock");
        }
        if (!tradingDays.empty()) SetTradingDayInfo(tradingDays);
        SetFromCore();
        return *this;
    ...
    Clock(const char* string_time, const char* format) {
        if (policy::IsTrading()) {
            THROW_ZERG_EXCEPTION("Trading Clock must be supplied with trading day list");
        }
        ...
        if (strptime(string_time, format, &timeinfo) == nullptr) {
            THROW_ZERG_EXCEPTION("Clock cannot properly parse your time string: " << string_time
                                                                                  << " with format: " << format);
        }
    ...
    Clock(const char* string_time, const char* format, std::vector<long>& list) {
        ...
        // parse it now
        if (strptime(string_time, format, &timeinfo) == nullptr)
            THROW_ZERG_EXCEPTION("Clock cannot properly parse time string: " << string_time
                                                                             << " with format: " << format);
        core.tv_sec = mktime(&timeinfo);
        core.tv_nsec = 0;
        SetTimeInfo();
    ...
    void SetTradingDayInfo(const std::vector<long>& tl) {
        ...
        if (tl.empty()) {
            THROW_ZERG_EXCEPTION("Clock does not accept empty trading day list");
        }

        if (tradingDays.front() > todayInt) {
            THROW_ZERG_EXCEPTION("TradingList not includes today");
    ...
    void SetTradingDayInfo(const std::string& filename) {
        ...
        if (!ifs.is_open()) {
            THROW_ZERG_EXCEPTION("cannot open info file" << filename);
        }
    ...
    class Clock operator+(const double interval_f) {
        ...
        if (tradingDayIndex + interval > tradingDays.size() - 1) {
            THROW_ZERG_EXCEPTION("clock trading day increment exceeds boundary");
        }
...
private:
    ...
    void AdjustTradingDay() {
        if (tradingDays.empty()) {
            if (policy::IsTrading()) {
                THROW_ZERG_EXCEPTION("TradingDay Clock cannot have empty trading day list");
            } else {
                return;
            }
        }
...
inline Clock<> HumanReadableTime(const std::string& literal) {
    ...
    if (found == std::string::npos) {
        THROW_ZERG_EXCEPTION("HumanReadableTime cannot recognize your time string: " << literal);
    }

    hour = time.substr(0, found);
    if (std::stoi(hour) > 12 && is_pm) {
        THROW_ZERG_EXCEPTION("HumanReadableTime finds your time string: " << literal << " to be illegal");
    }
    ...
    if (std::stoi(min) >= 60) {
        THROW_ZERG_EXCEPTION("HumanReadableTime illegal time: minute larger than 60");
    }

    if (std::stoi(sec) >= 60) {
        THROW_ZERG_EXCEPTION("HumanReadableTime illegal time: second larger than 60");
    }

    if (std::stoi(ms) >= 1000) {
        THROW_ZERG_EXCEPTION("HumanReadableTime illegal time: millisecond larger than 1000");
    }

    auto lit = ztool::Trim(hour) + ":" + ztool::Trim(min) + ":" + ztool::Trim(sec);
