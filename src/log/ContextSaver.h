...
public:
    ...
    ContextSaver& load(char* value){
        if((checkTypeNoThrow(TypeIndex::string_type) || checkTypeNoThrow(TypeIndex::char_array_type)) && value != nullptr){
            ++readPos;
            size_t len = 0;
...
private:
    void initMemoryLayout(size_t len){
        ...
        readPos = writePos = reinterpret_cast<uint8_t *>(start) + sizeof(ContextMeta);
        ContextMeta dummy;
        dummy.dataSize = len;
        *contextMeta = dummy;
    }
    ...
    bool checkTypeNoThrow(TypeIndex typeIndex){
        return *readPos == typeIndex;
    }
