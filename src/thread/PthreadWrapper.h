...
namespace frenzy {
    ...
    {
    public:
        PThread ( const int thread_no = -1, void* work=nullptr )
                : _running( false ), _thread_no(thread_no),p_work(work)
        {
        }
        ...
        void Create_BindList (  const bool  detached = false,
                                const bool  systemScope   = true,
            ...
            if ( ! _running )
            {
                ...
                if ( detached )
                {
                    ...
                    {
                        std::cerr << "(PThread) create : pthread_attr_setdetachstate (" << strerror( status ) << ")" << std::endl;
                        return;
                    }
                }
